...
public class S3Client
{
    ...
    public async Task<string> ListBucketsAsync()
    {
        ...
        var authorizationHeader = GenerateAuthorizationHeader(HttpMethod.Get, requestUri, iso8601Date, dateStamp, contentHash, host, sdkInvocationId, sdkRequest);
    ...
    private string GenerateAuthorizationHeader(HttpMethod method, string requestUri, string iso8601Date, string dateStamp, string contentHash, string host, string sdkInvocationId, string sdkRequest)
    {
        var canonicalRequest = CreateCanonicalRequest(method, requestUri, iso8601Date, contentHash, host, sdkInvocationId, sdkRequest);
        ...
        var authorizationHeader = $"AWS4-HMAC-SHA256 Credential={_accessKey}/{dateStamp}/s3/aws4_request, SignedHeaders=host;x-amz-date;x-amz-content-sha256;x-amz-sdk-invocation-id;x-amz-sdk-request, Signature={signature}";

        return authorizationHeader;
    }

    private string CreateCanonicalRequest(HttpMethod method, string requestUri, string iso8601Date, string contentHash, string host, string sdkInvocationId, string sdkRequest)
    {
        // Canonical request includes HTTP method, request URI, headers, and hashed payload
        var canonicalHeaders = $"host:{host}\nx-amz-date:{iso8601Date}\nx-amz-content-sha256:{contentHash}\nx-amz-sdk-invocation-id:{sdkInvocationId}\nx-amz-sdk-request:{sdkRequest}\n";
        var signedHeaders = "host;x-amz-date;x-amz-content-sha256;x-amz-sdk-invocation-id;x-amz-sdk-request";
